...
Lors de l'ouverture d'un fichier mobilisant des librairies non encore installées, RStudio affiche dans un bandeau au-dessus dudit fichier une invitation à installer les librairies manquantes. Un moyen autre moyen simple d'installer une librairie est d'utiliser le menu "packages" dans le panneau d'exploration (en bas à droite). Les librairies peuvent sinon être installées avec la commande `install.packages(sf)` pour la librairie `sf` par exemple.
...
Une fois que l'indicateur a été calculé individuellement pour tous les "actifs" du portefeuille, s'il y a plusieurs années ou variables calculées, les données apparaîssent sous la forme d'une **colonne de liste imbriquée** à l'objet d'origine. Si on a plusieurs indicateurs, on aura plusieurs colonnes avec des listes ayant une, deux ou plusieurs variables.
